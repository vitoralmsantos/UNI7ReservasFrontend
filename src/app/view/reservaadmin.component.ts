...
export class ReservaAdminComponent implements OnInit {
  ...
  constructor(private localService: LocalService, private categoriaService: CategoriaService,
    private reservaService: ReservaService, private usuarioService: UsuarioService, 
    ...
    this.categoriaService.getDisponibilidade(this.reserva.Data, this.reserva.Horario, this.reserva.Turno)
      .subscribe(response => {
        if (response === undefined) {
          ...
          this.categorias = []
          this.categorias.push({ Id: 0, Nome: '--Sem equipamento--', Comentario: '' })
          this.reserva.IdCategoria = 0
          this.categorias.push.apply(this.categorias, response.Elementos)
      ...
      .subscribe(response => {
        if (response === undefined) {
          ...
          this.usuarios = []
          this.usuarios.push.apply(this.usuarios, response.Elementos)
          this.reserva.IdUsuario = this.idUsuario
          this.usuDesabilitado = false
        }
        else {
          this.mostraErro(response.Detalhes)
          this.limparParcial()
  ...
  reservar(): void {
    ...
    if (this.tipoReserva == 1) {  //Reserva única
      ...
      if (this.ngbDateAteReserva === undefined) {
        this.mostraErro('Escolha a data limite da reserva.')
        return
      }

      let dia = String(100 + this.ngbDateAteReserva.day).substr(1, 2);
      let mes = String(100 + this.ngbDateAteReserva.month).substr(1, 2);
      this.reserva.DataAte = dia + '/' + mes + '/' + this.ngbDateAteReserva.year

      this.reservaService.addReservaPeriodo(this.reserva)
        .subscribe(response => {
          if (response === undefined) {
            this.mostraErro('Não foi possível realizar a reserva solicitada. Verifique conexão com Internet.')
          }
