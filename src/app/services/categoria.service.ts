...
export class CategoriaService {
  ...
  updateCategoria(categoria: Categoria): Observable<any> {
    ...
    if (categoria.Comentario === undefined)
      u.set('ComentarioReserva', '');
    else
      u.set('ComentarioReserva', categoria.Comentario.toString());

    let url = `${this.categoriaUrl}/atualizar`;
    return this.http.post<EntidadeResponse<Categoria>>(url, u.toString(), this.httpOptions)
  ...
  deleteCategoria(id: Number): Observable<any> {
    let url = `${this.categoriaUrl}/remover`;
    let u = new URLSearchParams();
